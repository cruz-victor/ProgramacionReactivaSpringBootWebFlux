...
class SpringBootWebfluxApirestApplicationTests {
	...
	*/
	...
	public void verTest() {
		...
		Producto producto = iProductoService.obtenerPorNombre("television 1").block();
	...
	public void crearTest() {
		...
		.jsonPath("$.producto.nombre").isEqualTo("Mesa comedor test")
	...
	public void crear2Test() {
		...
		.consumeWith(response->{
			...
			Assertions.assertThat(p.getId()).isNotEmpty();
			Assertions.assertThat(p.getNombre()).isEqualTo("Mesa comedor test");
	...
	public void editarTest() {
		Producto producto=iProductoService.findByNombre("television 2").block();
		Categoria categoria=iProductoService.findCategoriaByNombre("Electronico").block();
		
		Producto productoEditado=new Producto("Asus editado",700.00, categoria);
		...
		.expectBody()
		.jsonPath("$.id").isNotEmpty()
		.jsonPath("$.nombre").isEqualTo("Asus editado")
		.jsonPath("$.categoria.nombre").isEqualTo("Electronico");				
	}
	
	@Test
	public void eliminarTest() {
		Producto producto=iProductoService.findByNombre("television 7").block();
		client.delete()
		.uri(url+"/{id}",Collections.singletonMap("id", producto.getId()))
		.exchange()
